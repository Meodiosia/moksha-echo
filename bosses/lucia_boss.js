...
let _luciaLoading = false;
const _luciaReadyCallbacks = [];

function loadLuciaFrames(onReady) {
  // Already loaded - call immediately
  if(_luciaLoaded){ if(onReady) onReady(); return; }
  // Already loading - queue callback
  if(_luciaLoading){ _luciaReadyCallbacks.push(onReady); return; }

  if(typeof LUCIA_FRAME_COUNT === 'undefined'){
    console.error('[lucia] LUCIA_FRAME_COUNT not defined');
    _luciaLoaded = true;
    if(onReady) onReady();
    return;
  }
  _luciaLoading = true;
  if(onReady) _luciaReadyCallbacks.push(onReady);

  const keys = Object.keys(LUCIA_FRAME_COUNT);
  let totalFrames = 0;
  for(const k of keys) totalFrames += LUCIA_FRAME_COUNT[k];
  let loaded = 0;
  console.log('[lucia] loading frames, total=' + totalFrames);
  if(typeof window._LP !== 'undefined') window._LP.reg('lucia','露西亚动作帧', totalFrames);

  const _finish = function(){
    _luciaLoaded = true;
    _luciaLoading = false;
    console.log('[lucia] all frames ready');
    if(typeof window._LP !== 'undefined') window._LP.done('lucia');
    for(const cb of _luciaReadyCallbacks){ try{ cb(); }catch(e){} }
    _luciaReadyCallbacks.length = 0;
  };
  ...
  for(const key of keys){
    ...
    for(let i=0; i<cnt; i++){
      ...
      const _i = i;
      img.onload  = () => { loaded++; if(typeof window._LP!=='undefined') window._LP.tick('lucia',loaded,totalFrames); if(loaded === totalFrames) _finish(); };
      img.onerror = () => { loaded++; if(typeof window._LP!=='undefined') window._LP.tick('lucia',loaded,totalFrames); if(loaded === totalFrames) _finish(); };
...
const LUCIA_PHASES = {
  1: { name:'P1', label:'Phase 1', color:'#80FFB0' },
  2: { name:'P2', label:'Phase 2', color:'#FFD080' },
  3: { name:'P3', label:'Phase 3', color:'#FF6060' },
};
...
function spawnLucia(x, y){
  ...
  const maxHp = (window.GAME_DIFFICULTY && window.GAME_DIFFICULTY.bossMaxHp) || 3000;
...
function _checkPhaseTransition(){
  ...
  if(target > lucia.phase && lucia.pendingPhase === null){
    ...
    lucia.state = 'cheer';
    lucia.stateTimer = (LUCIA_FRAME_COUNT.cheer/30) * 0.7;  // cheer
    lucia.animT = 0;
...
function _luciaSpawnPhaseBurst(){
  ...
  if(isP3){
    ...
    lucia.p3IntroT = 5.0;       // 5
    _luciaSpawnSkillText('');
  }
...
function _luciaPickAttack(pdist){
  ...
  if(isP3 && pdist < 150){
    ...
    eligible.forEach(k => {
      boosted.push(k);
      if(meleeBoost.includes(k)) boosted.push(k);  //
    });
    eligible = boosted;
  }
...
function updateLucia(dt){
  ...
  if(lucia.state==='attack'){
    ...
    if(!mv.stationary && !mv.ranged && !mv.isCharge && mv.range > 0){
      ...
      if(lucia.atkPhaseT > 0.18 && lucia.atkPhaseT < hitStart){
        const desired = mv.range * 0.78;        //
        if(pdist > desired){
          const closingSpd = 320 * dt;          //
  ...
  const sweetXMin = isP3 ? 35  : 48;
  const sweetXMax = isP3 ? 90  : 115;
  const sweetYMax = isP3 ? 28  : 36;
  const tooFarDist = isP3 ? 110 : 145;
  ...
  const minPoseTime = isP3 ? 0.40 : 0.65;  // P3 稍长，减少频繁切换
  ...
  if(Math.abs(yDiffL) > 35){  // 从18提高到35，减少Y轴微调抖动
...
function luciaTakeDamage(dmg, kx, ky){
  ...
  lucia._parryCooldown = Math.max(0, lucia._parryCooldown - 0);  //
  ...
  if(!s._cracks){
    ...
    for(let i=0;i<N;i++){
      ...
      for(let j=0;j<SEGN;j++){
        ...
        cy += Math.sin(ang)*stepLen * ISO_Y_SCALE;   //
        segs.push({x:cx, y:cy});
      }
      s._cracks.push(segs);
    }
...
function _drawFxGiantSword(c, sx, sy, s, k){
  ...
  c.save();
  c.translate(sx, sy);
  c.translate(0, swordLen * 0.45);   //
  renderSword((1 - resK) * 0.85, true);    // residue
  c.globalCompositeOperation = 'lighter';
...
function _drawFxShadowRip(c, sx, sy, s, k){
  ...
  const alphaMul = s.alphaMul || 0.55;     //
  const mode = s.mode || 'in';              // 'in'
...
function drawLucia(){
  ...
  let animKey = 'idle';
  let animDur = LUCIA_FRAME_COUNT.idle / 24;   //
  let animT01;
  if(lucia.state==='dead'){
    animKey = 'death';
  ...
  const dh = (typeof BOSS_H!=='undefined'?BOSS_H:88) * 1.05;  // 1.4
...
function drawLuciaHUD(){
  ...
  ctx.fillText('LUCILLA', ctx.canvas ? ctx.canvas.width/2 : 400, 30);
  ctx.textAlign = 'left';
}
...
function _luciaTrySpawnClone(){
  ...
  const r = 40 + Math.random()*10;     // 40-50px
...
function _luciaUpdateClone(dt){
  ...
  if(cl.state === 'spawn'){
    cl.fade = Math.min(1, cl.fade + dt*5);   // ~0.2s
    ...
    if(lucia.state === 'attack' && cl.attackKey){
      ...
      if(mv && mv.ranged && mv.hitWin){
        if(!cl._cloneShot && lucia.atkPhaseT >= mv.hitWin[0] && lucia.atkPhaseT < mv.hitWin[1]){
          ...
          else if(mv.rangedType === 'ring_clones') { /* ring_clones handled elsewhere */}
...
function _luciaUpdateChargeGhost(dt){
  ...
  if(g.state === 'wait'){
    g.fade = Math.min(1, g.fade + dt*5);     // 0.2s
    g.waitT += dt;
...
function _luciaDrawSkillTexts(c){
  ...
  for(const t of lucia.skillTexts){
    ...
    const sx = t.x - ox();
    const sy = (t.y - oy()) * ISO_Y_SCALE - t.age * 30;   //
    c.save();
...
function _luciaUpdateLowHpRage(dt){
  ...
  if(pct < 0.22){
    ...
    if(lucia.p3IntroT <= 0) lucia.p3IntroT = 0;   //
  } else {
    lucia._tremorX *= 0.6;
    lucia._tremorY *= 0.6;
