/* RelicReward.show(relicIds, onSelect, onSkip); RelicReward.hide() */
(function(root){
  'use strict';
  ...
  function show(relicIds, onSelect, onSkip, opts){
    var ov = getOverlay();
    ov.innerHTML = '';
    ...
    title.textContent = (opts && opts.title) || '选择遗物';
    ov.appendChild(title);

    var sub = document.createElement('div');
    sub.className = 'rr-sub';
    sub.textContent = (opts && opts.sub) || '每次升级获得一件强化';
    ov.appendChild(sub);
    ...
    relicIds.forEach(function(id){
      ...
      card.innerHTML =
        (comboHint ? '<div class="rr-combo-hint">COMBO</div>' : '') +
        (ownedCount > 0 ? '<div class="rr-owned">x' + ownedCount + '</div>' : '') +        '<span class="rr-icon">' + relic.icon + '</span>' +
  ...
  function _showComboBanner(comboName){
    var bn = getBanner();
    bn.innerHTML =
      '<div class="rr-cb-title">COMBO ACTIVATED</div>' +
      '<div class="rr-cb-title" style="font-size:22px;margin:8px 0;">' + comboName + '</div>' +
      '<div class="rr-cb-sub">BONUS EFFECT UNLOCKED</div>';    bn.classList.add('rr-show');
    setTimeout(function(){
  ...
  function _checkComboHint(targetId){
    ...
    for(var cid in root.COMBO_DB){
      ...
      var wouldActivate = combo.requires.every(function(rid){
        ...
        return root.RelicManager.has(rid);      //
      });
