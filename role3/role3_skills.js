// ── role3_skills.js ───────────────────────────────────────────
// Role3（飞剑使）剑数核心 gameplay - 恢复完整版本
// CD 配置：小技能 12s / 冲刺格挡 3s / 大招 25s
...
window.role3 = {
  ...
  isBusy(){
    // 允许在 attack 状态收尾期（最后 35%）被技能打断（移动端按键反馈更灵敏）
  ...
  update(dt){
    ...
    // V 技能：集结剑阵期间，覆写飞剑位置（紧贴 22r 圈）+ 加速旋转 + 吐粒子
    if(typeof role3Swords !== 'undefined'){
      for(const s of role3Swords){
        if(!s._vFormation) continue;
        s._vFormPhase = (s._vFormPhase || 0) + dt * 14;   // 高速旋转
  ...
  doUltimate(){
    ...
    player.state = 'attack';
    player.stateTimer = 1.8;
    player.attackDur = 1.8;
    comboIndex = 1;
    for(let i=0; i<12; i++){
  ...
  doBladeStep(){
    ...
    if(this.swordCount() < 2) return;       // 至少 2 把剑
    this.cdB = 8.0;
    ...
    player.x = tx;
    player.y = ty;
    player.facing = dirX >= 0 ? -1 : 1;     // 面向 boss
    player.state = 'attack';
    player.stateTimer = 0.50;
    player.attackDur = 0.50;
    comboIndex = 2;

    // 所有 orbit 飞剑同时朝 boss 方向刺出（剑墙），50ms 后回收
  ...
  doSanctuary(){
    ...
    player.state = 'attack';
    player.stateTimer = 1.20;
    player.attackDur = 1.20;
    comboIndex = 1;
    if(target) player.facing = target.x > player.x ? 1 : -1;

    // 拷贝当前所有 orbit 飞剑列表（射剑期间剑会切 shoot 状态，不能在循环中遍历）
    const orbits = role3SwordsAPI.getOrbits().slice();
    const N = orbits.length;
    if(N === 0) return;

    // 第一阶段：0.0~0.45s — 集结阶段
    //   把所有飞剑拉到玩家周围紧密的小圈（r 18，等间隔），
    //   期间持续吐光粒子，地面冲击波
    const tightR = 22;
    orbits.forEach((s, i) => {
      // 锁定阶段标记，避免 update 里被风暴/普通 orbit 覆盖
      s._vFormation = true;
...
window.addEventListener('keydown', e => {
  if(currentChar !== 3 || !window.role3) return;
  if(e.code === 'KeyO'){
